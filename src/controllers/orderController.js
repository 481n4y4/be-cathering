...
const checkout = async (req, res) => {
  try {
    ...
    // Get user's cart
    const cart = await Keranjang.findOne({ user: userId }).populate(
      "items.produk",
    );
  ...
  try {
    ...
    const parameter = {
      ...
      customer_details: {
        ...
        phone: order.user.no_telepon?.toString() || "",
        billing_address: {
          address: order.user.alamat || "",
        },
      },
      item_details: order.items.map((item) => ({
        id: item.produk._id.toString(),
        price: item.harga,
    ...
    // Update order status - gunakan 'Diproses' karena 'Paid' tidak ada di enum
    order.status = "Diproses"; // ← Ubah dari 'Paid' menjadi 'Diproses'
    await order.save();
